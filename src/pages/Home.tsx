        ...
        <div className="relative z-10 max-w-4xl">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <button 
                onClick={() => { setSelectedPuja(PUJAS[0]); setIsBookingOpen(true); }}
                className="w-full sm:w-auto px-10 py-4 bg-primary text-white rounded-full font-bold text-lg hover:bg-primary/90 transition-all shadow-xl shadow-primary/30 flex items-center justify-center gap-2"
              >
            ...
            >
              <Link to={`/puja/${puja.id}`} className="block">
                <div className="relative h-48 overflow-hidden">
                  <img src={puja.image} alt={puja.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" referrerPolicy="no-referrer" />
          ...
          {PANDITS.map((pandit) => (
            <div key={pandit.id} className="premium-card p-8 flex flex-col items-center text-center">
              <div className="relative mb-6">
                <img src={pandit.image} alt={pandit.name} className="w-24 h-24 rounded-full object-cover border-4 border-slate-50 shadow-lg" referrerPolicy="no-referrer" />
                {pandit.verified && (
              ...
              <Link 
                to={`/pandit/${pandit.id}`}
                className="w-full py-3 border border-secondary text-secondary rounded-xl font-bold hover:bg-secondary hover:text-white transition-all block"
              >
                View Profile
              </Link>
            </div>
      ...
      <section className="py-20 px-6">
        <div className="max-w-5xl mx-auto saffron-gradient rounded-3xl p-12 text-center text-white relative overflow-hidden shadow-2xl shadow-primary/20">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4 relative z-10">
            <button 
              onClick={() => { setSelectedPuja(PUJAS[0]); setIsBookingOpen(true); }}
              className="px-10 py-4 bg-white text-primary rounded-full font-bold text-lg hover:shadow-xl transition-all"
            >
              Book Your Puja Now
            </button>
            <button 
